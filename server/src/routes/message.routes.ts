...
messageRouter.post("/messages", async (req: Request, res: Response) => {
  try {
    ...
    const newMessage = {
      ...
      senderId: userId, // Explicit senderId mező
      sender: userId, // Backward compatibility
      receiverId: receiverId ? new ObjectId(receiverId) : null,
      content,
      createdAt: new Date(),
      timestamp: new Date(), // Explicit timestamp mező a frontend számára
      isRead: false,
    }
    ...
    // Explicit senderId és timestamp mezők a válaszban
    res.status(201).json({
      ...newMessage,
      _id: result.insertedId,
      senderId: userId,
...
messageRouter.get("/conversations/:conversationId/messages", async (req: Request, res: Response) => {
  try {
    ...
    // Explicit senderId és timestamp mezők hozzáadása minden üzenethez
